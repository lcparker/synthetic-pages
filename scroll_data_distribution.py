...
def intensity_distribution_of_page(volume: Nrrd, mask: Nrrd, page_number: int) -> gaussian_kde:
...
def make_distribution_files():
    ...
    for d in paths:
        ...
        if not np.all(np.array(vol.volume.shape) == np.array([256,256,256])):
            ...
            continue
        print(f"loaded volume (shape {vol.volume.shape}) and mask (shape {mask.volume.shape})")
...
def grep_volume_from_directory(directory: str | Path) -> Nrrd:
    files = [p for p in Path(directory).iterdir() if p.name.startswith('volume') and p.suffix == '.nrrd']
    if len(files) > 0:
        volume_file = files[0]
...
def make_synthetic_page_volume(data_folder: str | Path = 'hari-cubes/cube_0_2408_4560/', 
    ...
    labels = page_meshes_to_volume(meshes, ground_truth_volume.metadata['sizes'][0], .05, volume_bbox)
